...
def make_qwe_envs(flags):
    ...
    if flags.env_type == "linear":
        for i in range(envs_num):
            envs.append(
                make_environment(
                    train_data[i::envs_num], 
                    train_labels[i::envs_num], 
        ...
        raise Exception("Unsupported env_type")

    # 最后添加验证集环境
    envs.append(make_environment(val_data, val_labels, 0.9))
...
def make_mnist_envs(flags):
    mnist = datasets.MNIST('~/datasets/mnist', train=True, download=True) # download
    mnist_train = (mnist.data[:flags.data_num], mnist.targets[:flags.data_num])
    mnist_val = (mnist.data[flags.data_num:], mnist.targets[flags.data_num:])
    rng_state = np.random.get_state()
    np.random.shuffle(mnist_train[0].numpy())
...
class LYDataProvider(object):
    ...
    def fetch_test(self):
        pass
# 类irmDataProver 继承 LYDataProvider
class IRMDataProvider(LYDataProvider):
    def __init__(self, flags):
        super(IRMDataProvider, self).__init__() # 父类的构造函数

    def preprocess_data(self):
        self.train_x, self.train_y, self.train_g, self.train_c= concat_envs(self.envs[:-1]) #concat其余数据集（1）
        self.test_x, self.test_y, self.test_g, self.test_c= concat_envs(self.envs[-1:])# 最后一个用于训练（2）

    def fetch_train(self):
        return self.train_x, self.train_y, self.train_g, self.train_c # （1）

    def fetch_test(self):
        return self.test_x, self.test_y, self.test_g, self.test_c # （2）

#* 改动2 QWE_LearningLayerDataProvider
class QWE_LYDP(IRMDataProvider):
    def __init__(self, flags):
        super(QWE_LYDP, self).__init__(flags)
        self.flags = flags
        self.envs = make_qwe_envs(flags) # 改动3声明；这部数据出口
        self.preprocess_data()


# CMNIST_LYDP 继承 IRMDataProvider
